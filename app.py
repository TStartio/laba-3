...
def contrast():
    ...
    def brightness():
        ...
        # Get the brightness factor from the request
        factor1 = float(request.form.get('factor1'))
        factor2 = float(request.form.get('factor2'))
        factor3 = float(request.form.get('factor3'))
        # Modify the brightness of the image
        img_brighter = brightness(img, factor1, factor2, factor3)

        # Convert the modified image back to bytes
        img_bytes = Image.fromarray(img_brighter).tobytes()
        ...
        # change brightness
        # Load the image
        # Convert the image to an array to manipulate the pixels
        im_array = array(contrasted_img)

        # Define the function to modify the brightness of the image
        def brightness(image, factor1, factor2, factor3):
            # Multiply each color channel by the brightness factor
            r, g, b = image[:, :, 0], image[:, :, 1], image[:, :, 2]
            r = (r * factor1).clip(0, 255)
            g = (g * factor2).clip(0, 255)
            b = (b * factor3).clip(0, 255)
            # Combine the channels back into an RGB image
            return np.array([r, g, b]).transpose((1, 2, 0)).astype('uint8')

        # Display the input and output values as a table
        #table = plt.table(cellText=[im_array[0], im_brighter[0]], cellLoc='center', loc='center')
        #plt.axis('off')
